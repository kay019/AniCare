...
<body>
	...
	<div id="form_wrapper">
		<form id="detailViewForm" method="post">
			...
			<div class="content_wrapper">
				...
				<div class="wrapper">
					...
					<div class="input_area div_in_wrapper">
						<input type="text" name="title" value="${requestScope.selectFreeBoardPost.title }" readonly>
					</div>
				</div>
				<!-- wrapper1 area end -->
				...
				<div class="wrapper">
					...
					<div class="input_area div_in_wrapper">
						<c:set var="post" value="${requestScope.selectFreeBoardPost }" />
				...
				<div class="wrapper">
					...
					<div class="input_area div_in_wrapper">
						<input type="email" name="email" value="${requestScope.selectFreeBoardPost.email }" readonly>
					</div>
				</div>
				<!-- wrapper3 area end -->
				...
				<div class="wrapper">
					...
					<div class="input_area div_in_wrapper">
						<textarea name="content" style="font-size: 15px; margin-top: 5px;" readonly>${requestScope.selectFreeBoardPost.content }</textarea>
					</div>
				...
				<c:if test="${requestScope.selectFreeBoardPost.userId == sessionScope.loginUser.userId }">
			...
			<input type="hidden" name="bCode" value="${requestScope.selectFreeBoardPost.bCode }"/>
			<input type="hidden" name="cc" value="${requestScope.replyInfoFreeBoard['commentCountFreeBoard'] }" />
		</form>
	</div><br/>
	<div id="comment_wrapper">
		...
		<c:if test="${requestScope.replyInfoFreeBoard['commentCountFreeBoard'] > 0}">
			<table border="1" id="reply_table">
				...
				<c:forEach var="reply" items="${requestScope.replyInfoFreeBoard['replyListFreeBoard'] }">
					<tr>
	...
	<script>
			...
			switch($flag.text()) {
				case '삭제하기':
					if(window.confirm('삭제하시겠습니까?')) {
						$("#detailViewForm").attr('action', '${pageContext.request.contextPath }/deleteFreeBoard.bo');
						$("#detailViewForm").submit();
					}
					break;
				case '수정완료':
					if (window.confirm('수정하시겠습니까?')) {
						$("#detailViewForm").attr('action', '${pageContext.request.contextPath }/updateFreeBoard.bo');
						$("#detailViewForm").submit();
					}
					break;
			}
		...
		$("#commentWriteBtn").click(function() {
			...
			var path = "${pageContext.request.contextPath }/insertReplyForFreeBoard.bo?";
			path += "userId=" + '${sessionScope.loginUser.userId}';
			path += "&content=" + $("#content").val();
			path += "&bCode=" + ${requestScope.selectFreeBoardPost.bCode };
			path += "&cc=" + ${requestScope.replyInfoFreeBoard['commentCountFreeBoard'] + 1};
		...
		$(".reply_deleteBtn").mouseenter(function() {
			...
			var cc = "${requestScope.replyInfoFreeBoard['commentCountFreeBoard']}";
			var refBcode = $("#refBcode").val();
			switch(flag) {
			case '삭제':
				if(window.confirm('삭제하시겠습니까?')) {
					var path = "${pageContext.request.contextPath}/deleteReplyFreeBoard.bo?"
					path += "bCode=" + bCode;
			...
			case '완료':
				...
				var path = "${pageContext.request.contextPath}/updateReplyFreeBoard.bo?";
				path += "bCode=" + bCode;
