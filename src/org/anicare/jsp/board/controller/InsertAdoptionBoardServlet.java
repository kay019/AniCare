...
/**
 * Servlet implementation class InsertAdoptionBoardServlet
 */
@WebServlet("/insertAdoptionBoard.bo")
public class InsertAdoptionBoardServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public InsertAdoptionBoardServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int result = new BoardService().insertAdoptionBoard(insertBoard);
		
		// List Load Through Servlet
		// Pagenation Processing
		if (result > 0) {
			response.sendRedirect(request.getContextPath() + "/selectAdoptionBoardList.bo");
		}
		else {
			request.setAttribute("message", "입양후기 작성 실패");
